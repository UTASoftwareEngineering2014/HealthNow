...
public class ExerciseFragment extends Fragment
{
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState)
	{
		...
		Button calculateCalories=(Button)rootView.findViewById(R.id.btn_calculateCalories);
		Button searchExercise=(Button)rootView.findViewById(R.id.btn_searchExercise);
		Button savedExercise=(Button)rootView.findViewById(R.id.btn_savedexercise);
		Button resetTimer=(Button)rootView.findViewById(R.id.btn_resetTimer);
		ToggleButton timer_toggle=(ToggleButton)rootView.findViewById(R.id.togbtn_timerToggle);
		Button startPedometer=(Button)rootView.findViewById(R.id.btn_startPedometer);
		Button heartRateMonitor=(Button)rootView.findViewById(R.id.btn_heartRateMonitor);
		Context exerciseContext=getActivity().getApplicationContext();
		View exerciseView=rootView;
		buttonlistener fortimer=new buttonlistener(exerciseView, exerciseContext);
		calculateCalories.setOnClickListener(new buttonlistener(exerciseView,exerciseContext)); 
		searchExercise.setOnClickListener(new buttonlistener(exerciseView, exerciseContext));
		savedExercise.setOnClickListener(new buttonlistener(exerciseView, exerciseContext));
		timer_toggle.setOnCheckedChangeListener(fortimer);
		resetTimer.setOnClickListener(fortimer);
		startPedometer.setOnClickListener(new buttonlistener(exerciseView, exerciseContext));
		heartRateMonitor.setOnClickListener(new buttonlistener(exerciseView, exerciseContext));
		return rootView;
	...
	class buttonlistener implements View.OnClickListener,CompoundButton.OnCheckedChangeListener
	{
		...
		View exerciseView;Context exerciseContext;
		...
		public void onClick(View v) 
		{
			...
			switch(v.getId())
			{
				...
				{
					PopupMenu pum = new PopupMenu(exerciseContext, exerciseView.findViewById(R.id.btn_calculateCalories));
				...
				{
					Intent searchexerciseIntent = new Intent(getActivity(), Search_Exercise.class);
				...
				case R.id.btn_savedexercise:
				{
					...
					Intent savedExerciseIntent = new Intent(getActivity(), Saved_Exercise.class);
				...
				case R.id.btn_resetTimer:
				{
					...
					Chronometer timer=(Chronometer)exerciseView.findViewById(R.id.cm_exerciseTimer);
					timer.setBase(SystemClock.elapsedRealtime());
					ToggleButton timer_toggle=(ToggleButton)exerciseView.findViewById(R.id.togbtn_timerToggle);
		            if(!(timer_toggle.isChecked()))timer.stop();
		            timeWhenStopped = 0;
				...
				case R.id.btn_heartRateMonitor:
				{
					...
					//Context context = getActivity().getApplicationContext();
					Intent startHeartRateMnitor=new Intent(getActivity(),HeartRateMonitor.class);
...
class runningDialog extends DialogFragment 
{
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState)
    {
        ...
        View promptsView = getActivity().getLayoutInflater().inflate(R.layout.running_layout, null);
        // Inflate and set the layout for the dialog
        // Pass null as the parent view because its going in the dialog layout
        builder.setView(promptsView);
        final EditText et_time=(EditText)promptsView.findViewById(R.id.running_time);
 	   final EditText et_distance=(EditText)promptsView.findViewById(R.id.running_distance);
 	   final EditText et_weight=(EditText)promptsView.findViewById(R.id.running_weight);
               ...
               builder.setPositiveButton("Calculate", new DialogInterface.OnClickListener() {
                   ...
                   public void onClick(DialogInterface dialog, int id) 
                   {
                	...
                	if(type.equalsIgnoreCase("r"))
                	{
                	   ...
                	   try
                	   {
                		   double time=(Integer.parseInt(et_time.getText().toString()))/60.0;
                		   double dist=Double.parseDouble(et_distance.getText().toString());
                		   double speed=dist/time;
                		   double weight=Double.parseDouble(et_weight.getText().toString());
                		   int count=0;
                		   if(speed>=2 && speed<2.5)
                		   {
                			   cal=.0175*2.5*(weight/2.2)*time*60;
                			   Log.e("cal", String.valueOf(cal));
                		   }
                		   else if(speed>=2.5 && speed<3)
                		   {
                			   cal=.0175*3*(weight/2.2)*time*60;
                		   }
                		   else if(speed>=3 && speed<3.5)
                		   {
                			   cal=.0175*3.5*(weight/2.2)*time*60;
                		   }
                		   else if(speed>=3.5 && speed<4)
                		   {
                			   cal=.0175*4*(weight/2.2)*time*60;
                		   }
                		   ...
                		   {
                			   cal=.0175*10*(weight/2.2)*time*60;
                		   }
                		   else if(speed>=6.7 && speed<7)
                		   {
                			   cal=.0175*11*(weight/2.2)*time*60;
                		   }
                		   else if(speed>=7 && speed<7.5)
                		   {
                			   cal=.0175*11.5*(weight/2.2)*time*60;
                		   }
                		   else if(speed>=7.5 && speed<8)
                		   {
                			   cal=.0175*12.5*(weight/2.2)*time*60;
                		   }
                		   else if(speed>=8 && speed<8.6)
                		   {
                			   cal=.0175*13.5*(weight/2.2)*time*60;
                		   }
                		   else if(speed>=8.6 && speed<9)
                		   {
                			   cal=.0175*14*(weight/2.2)*time*60;
                		   }
                		   else if(speed>=9 && speed<10)
                		   {
                			   cal=.0175*15*(weight/2.2)*time*60;
                		   }
                		   else if(speed>=10 && speed<10.9)
                		   {
                			   cal=.0175*16*(weight/2.2)*time*60;
                		   }
                		   else if(speed>=10.9 && speed<12)
                		   {
                			   cal=.0175*18*(weight/2.2)*time*60;
                		   }
                		   else
                		   {
                			   count=1;
                		   ...
                		   if(count==0)
                		   {
                			   ...
                			   cal=cal/100.0;
                			   CharSequence text = "Calories Burnt:"+String.valueOf(cal+" calories\nGood Job!");
                			   int duration = Toast.LENGTH_LONG;
                			   Toast toast = Toast.makeText(context, text, duration);
                			   toast.show();
                		   }
                	...
                	else if(type.equalsIgnoreCase("c"))
                	{
                		try
                 	   {
                 		   ...
                 		   {
                 			   cal=.0175*12*(weight/2.2)*time*60;
                 		   }
                 		   else
                 		   {
                 			  count=1;
